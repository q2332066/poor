...
layui.define(["layer", "setter", "layRouter"], function (f) {
    var h = layui.jquery;
    var k = layui.layer;
    var d = layui.setter;
    ...
    n.activeNav = function (q) {
        ...
        var t = h(e + '>.layui-nav a[href="#' + q + '"]');
        if (t.length === 0) {
            return console.warn(q + " not found")
        }
        var s = h(".layui-layout-admin").hasClass("admin-nav-mini");
        ...
        if (!s) {
            ...
            v.slideDown("fast", function () {
                if (h(this).is(v.last())) {
                    v.css("display", "");
                    var w = t.offset().top + t.outerHeight() + 30 - n.getPageHeight();
                    var x = 50 + 65 - t.offset().top;
                    if (w > 0) {
                        h(e).animate({"scrollTop": h(e).scrollTop() + w}, 300)
                    } else {
                        if (x > 0) {
                            h(e).animate({"scrollTop": h(e).scrollTop() - x}, 300)
                        }
                    }
                }
            })
        ...
        h(j + '>.layui-nav>.layui-nav-item>a[nav-bind="' + r.attr("nav-id") + '"]').parent().addClass("layui-this")
    };
    ...
    n.getLayerData = function (i, q) {
        if (i === undefined) {
            i = parent.layer.getFrameIndex(window.name);
            if (i === undefined) {
                return null
            } else {
                return parent.layui.admin.getLayerData(parseInt(i), q)
            }
        } else {
            if (isNaN(i)) {
                i = n.getLayerIndex(i)
    ...
    n.putLayerData = function (q, s, i) {
        if (i === undefined) {
            i = parent.layer.getFrameIndex(window.name);
            if (i === undefined) {
                return
            } else {
                return parent.layui.admin.putLayerData(q, s, parseInt(i))
            }
        } else {
            if (isNaN(i)) {
                i = n.getLayerIndex(i)
        ...
        n.ajax({
            url: i, dataType: "html", async: false, success: function (w) {
                ...
                if (u === true || u === "true") {
                    ...
                    w = n.util.tpl(t.html(), y, d.tplOpen, d.tplClose);
                    for (var x in v) {
                        w = w.replace("${" + x + "}", v[x])
                    }
                }
                r.children(".layui-layer-content").html(w);
                n.renderTpl("#layui-layer" + q + " [ew-tpl]");
                s && s(r[0], q)
            }
        })
    };
    n.alert = function (q, i, r) {
        if (typeof i === "function") {
            r = i;
            i = {}
        }
        ...
        return k.alert(q, i, r)
    };
    n.confirm = function (r, i, s, q) {
        if (typeof i === "function") {
            q = s;
            s = i;
            i = {}
        ...
        return k.confirm(r, i, s, q)
    };
    n.prompt = function (i, q) {
        if (typeof i === "function") {
            q = i;
            i = {}
        }
    ...
    n.req = function (i, r, s, t, q) {
        ...
        return n.ajax(h.extend({url: (d.baseServer || "") + i, data: r, type: t, dataType: "json", success: s}, q))
    };
    n.ajax = function (u) {
        var r = n.util.deepClone(u);
        if (!u.dataType) {
            u.dataType = "json"
        }
        if (!u.headers) {
        ...
        var t = d.getAjaxHeaders(u.url);
        if (t) {
            for (var q = 0; q < t.length; q++) {
                if (u.headers[t[q].name] === undefined) {
                    u.headers[t[q].name] = t[q].value
                }
            }
        }
        u.async = false;
        ...
        u.success = function (i, v, x) {
            var w = d.ajaxSuccessBefore(n.parseJSON(i), u.url, {
                param: r, reload: function (y) {
                    n.ajax(h.extend(true, r, y))
                }, update: function (y) {
                    i = y
                }, xhr: x
            });
        ...
        u.error = function (v, i) {
            u.success({code: v.status, msg: v.statusText}, i, v)
        };
        if (layui.cache.version && (!d.apiNoCache || u.dataType.toLowerCase() !== "json")) {
            if (u.url.indexOf("?") === -1) {
                u.url += "?v="
            } else {
                u.url += "&v="
            ...
            if (layui.cache.version === true) {
                u.url += new Date().getTime()
            } else {
                u.url += layui.cache.version
            }
        }
        return h.ajax(u)
    };
    n.parseJSON = function (q) {
        if (typeof q === "string") {
            try {
                return JSON.parse(q)
            } catch (i) {
    ...
    n.showLoading = function (t, s, q, r) {
        if (t !== undefined && (typeof t !== "string") && !(t instanceof h)) {
            s = t.type;
            q = t.opacity;
            r = t.size;
            t = t.elem
        ...
        var i = ['<div class="ball-loader ' + r + '"><span></span><span></span><span></span><span></span></div>', '<div class="rubik-loader ' + r + '"></div>', '<div class="signal-loader ' + r + '"><span></span><span></span><span></span><span></span></div>', '<div class="layui-loader ' + r + '"><i class="layui-icon layui-icon-loading layui-anim layui-anim-rotate layui-anim-loop"></i></div>'];
        h(t).addClass("page-no-scroll");
        h(t).scrollTop(0);
        var u = h(t).children(".page-loading");
        if (u.length <= 0) {
            h(t).append('<div class="page-loading">' + i[s - 1] + "</div>");
            u = h(t).children(".page-loading")
        }
        if (q !== undefined) {
            u.css("background-color", "rgba(255,255,255," + q + ")")
        }
        u.show()
    };
    n.removeLoading = function (q, s, i) {
        if (q === undefined) {
            q = "body"
        }
        if (s === undefined) {
        ...
        h(q).removeClass("page-no-scroll")
    };
    n.putTempData = function (r, s, q) {
        var i = q ? d.tableName : d.tableName + "_tempData";
        if (s === undefined || s === null) {
            if (q) {
                layui.data(i, {key: r, remove: true})
            } else {
                layui.sessionData(i, {key: r, remove: true})
            }
        } else {
            if (q) {
                layui.data(i, {key: r, value: s})
            } else {
                layui.sessionData(i, {key: r, value: s})
            }
        }
    };
    n.getTempData = function (r, q) {
        if (typeof r === "boolean") {
            q = r;
            r = undefined
        }
        var i = q ? d.tableName : d.tableName + "_tempData";
        var s = q ? layui.data(i) : layui.sessionData(i);
        if (!r) {
            return s
        }
        return s ? s[r] : undefined
    };
    n.rollPage = function (s) {
        var q = h(l + ">.layui-tab-title");
        var r = q.scrollLeft();
        if ("left" === s) {
    ...
    n.closeThisTabs = function (i) {
        ...
        if (!i) {
            ...
            if (i === q.find("li").first().attr("lay-id")) {
                return k.msg("主页不能关闭", {icon: 2})
            }
            q.find('li[lay-id="' + i + '"]').find(".layui-tab-close").trigger("click")
        }
    };
    ...
    n.changeTheme = function (w, v, q, u) {
        if (!q) {
            n.putSetting("defaultTheme", w)
        }
        if (!v) {
            v = top
        }
        ...
        for (var r = 0; r < x.length; r++) {
            n.changeTheme(w, x[r], true, false)
        }
    };
    ...
    n.btnLoading = function (q, r, s) {
        ...
        if (s) {
            ...
            i.prepend('<span class="ew-btn-loading-text"><i class="layui-icon layui-icon-loading layui-anim layui-anim-rotate layui-anim-loop"></i>' + r + "</span>");
    ...
    n.parseLayerOption = function (r) {
        ...
        for (var q in i) {
            if (!i.hasOwnProperty(q) || !r[q]) {
                continue
            }
            try {
                if (/^[a-zA-Z_]+[a-zA-Z0-9_]+$/.test(r[q])) {
                    r[q] += "()"
                }
                r[q] = new Function(i[q], r[q])
            } catch (t) {
                r[q] = undefined
            }
        }
        if (r.content && (typeof r.content === "string") && r.content.indexOf("#") === 0) {
            if (h(r.content).is("script")) {
                r.content = h(r.content).html()
            } else {
                r.content = h(r.content)
            }
        }
        if (r.type === undefined && r.url === undefined) {
            r.type = 2
        }
        return r
    };
    n.strToWin = function (t) {
        var s = window;
        if (!t) {
            return s
        }
    ...
    n.hideTableScrollBar = function () {
        if (n.getPageWidth() <= 768) {
            return
        }
        var i = d.pageTabs ? h(l + ">.layui-tab-content>.layui-tab-item.layui-show") : h(b);
        if (window.hsbTimer) {
    ...
    n.resizeTable = function (i) {
        setTimeout(function () {
            var q = d.pageTabs ? h(l + ">.layui-tab-content>.layui-tab-item.layui-show") : h(b);
            q.find(".layui-table-view").each(function () {
                var r = h(this).attr("lay-id");
                layui.table && layui.table.resize(r)
            })
        }, i === undefined ? 0 : i)
    };
    n.hasPerm = function (t) {
        var s = d.getUserAuths();
        if (s) {
    ...
    n.renderPerm = function () {
        h("[perm-show]").each(function () {
            if (!n.hasPerm(h(this).attr("perm-show"))) {
                h(this).remove()
            }
        })
    };
    n.events = {
        flexible: function () {
            ...
            n.strToWin(i.window).layui.admin.popupRight(h.extend({
                id: "layer-theme",
                url: i.url || "components/tpl/theme.html"
            }, n.parseLayerOption(i)))
        }, note: function () {
            var i = n.util.deepClone(h(this).data());
            n.strToWin(i.window).layui.admin.popupRight(h.extend({
                id: "layer-note",
                url: i.url || "components/tpl/note.html"
            }, n.parseLayerOption(i)))
        }, message: function () {
            var i = n.util.deepClone(h(this).data());
            n.strToWin(i.window).layui.admin.popupRight(h.extend({
                id: "layer-notice",
                url: i.url || "components/tpl/message.html"
            }, n.parseLayerOption(i)))
        }, psw: function () {
            var i = n.util.deepClone(h(this).data());
            n.strToWin(i.window).layui.admin.open(h.extend({
                ...
                title: "修改密码",
                shade: 0,
                url: i.url || "components/tpl/password.html"
            }, n.parseLayerOption(i)))
        }, logout: function () {
            ...
            function i() {
                if (q.ajax) {
                    var r = k.load(2);
                    n.req(q.ajax, function (t) {
                        k.close(r);
                        if (q.parseData) {
                            try {
                                var s = new Function("res", q.parseData);
                                t = s(t)
                            } catch (u) {
                                console.error(u)
                            }
            ...
            if (false === q.confirm || "false" === q.confirm) {
                return i()
            }
            n.strToWin(q.window).layui.layer.confirm(q.content || "确定要退出登录吗？", h.extend({
                title: "温馨提示",
                skin: "layui-layer-admin",
                shade: 0.1
            }, n.parseLayerOption(q)), function () {
                i()
            })
        }, open: function () {
            var i = n.util.deepClone(h(this).data());
            ...
            var x = "layui-icon-screen-full", q = "layui-icon-screen-restore";
            var w = h(this).find("i");
            var t = document.fullscreenElement || document.msFullscreenElement || document.mozFullScreenElement || document.webkitFullscreenElement || false;
            if (t) {
                var s = document.exitFullscreen || document.webkitExitFullscreen || document.mozCancelFullScreen || document.msExitFullscreen;
                if (s) {
                    s.call(document)
                } else {
                ...
                w.addClass(x).removeClass(q)
            } else {
                var i = document.documentElement;
                var r = i.requestFullscreen || i.webkitRequestFullscreen || i.mozRequestFullScreen || i.msRequestFullscreen;
                if (r) {
                    r.call(i)
                } else {
                    if (window.ActiveXObject) {
    ...
    n.chooseLocation = function (u) {
        ...
        if (t === undefined) {
            t = "https://webapi.amap.com/maps?v=1.4.14&key=006d995d433058322319fa797f2876f5"
        }
        var D = false, A;
        var w = function (F, E) {
        ...
        var i = function () {
            ...
            h("#ew-map-select-pois").off("click").on("click", ".ew-map-select-search-list-item", function () {
                ...
                var J = h(this).data("lng");
                var K = h(this).data("lat");
                var I = h(this).find(".ew-map-select-search-list-item-title").text();
                var H = h(this).find(".ew-map-select-search-list-item-address").text();
                A = {name: I, address: H, lat: K, lng: J};
                D = true;
                G.setZoomAndCenter(v, [J, K])
            });
            ...
            var F = h("#ew-map-select-input-search");
            F.off("input").on("input", function () {
                var H = h(this).val();
                var I = h("#ew-map-select-tips");
                if (!H) {
                    I.html("");
    ...
    n.cropImg = function (s) {
        ...
        layui.use(["Cropper", "upload"], function () {
            ...
            n.open({
                title: i, area: "665px", type: 1, content: C, success: function (D, E) {
    ...
    n.util = {
        Convert_BD09_To_GCJ02: function (q) {
            ...
            var i = q.lng - 0.0065, u = q.lat - 0.006;
        ...
        }, animateNum: function (u, q, s, v) {
            q = q === null || q === undefined || q === true || q === "true";
            s = isNaN(s) ? 500 : s;
            v = isNaN(v) ? 100 : v;
            var t = function (y) {
                var w = "";
            ...
            for (var s in t) {
                ...
                var u = t[s], r = n.util.isClass(u);
                if (r === "Object") {
                    i[s] = arguments.callee(u)
                } else {
                    if (r === "Array") {
                        i[s] = arguments.callee(u)
                    } else {
            ...
            if (!i) {
                var q = h(".layui-layout-admin>.layui-body");
                i = q.children(".layui-tab").children(".layui-tab-content").children(".layui-tab-item.layui-show");
                if (i.length === 0) {
                    i = q.children(".layui-body-header.show+div");
                    if (i.length === 0) {
                        i = q
                    }
                }
            ...
            var q = {
                exp: function (u) {
                    ...
                    var v = ["#([\\s\\S])+?", "([^{#}])*?"][w || 0];
                    return q.exp((u || "") + i + v + t + (x || ""))
                }, escape: function (u) {
                    return String(u || "").replace(/&(?!#?[a-zA-Z0-9]+;)/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/'/g, "&#39;").replace(/"/g, "&quot;")
                }, error: function (v, u) {
                    console.error("Laytpl Error：" + v + "\n" + (u || ""))
                }, parse: function (v, w) {
                    var u = v;
                    try {
                        var y = q.exp("^" + i + "#"), z = q.exp(t + "$");
                        v = v.replace(q.exp(i + "#"), i + "# ").replace(q.exp(t + "}"), "} " + t).replace(/\\/g, "\\\\").replace(q.exp(i + "!(.+?)!" + t), function (A) {
                            A = A.replace(q.exp("^" + i + "!"), "").replace(q.exp("!" + t), "").replace(q.exp(i + "|" + t), function (B) {
                                return B.replace(/(.)/g, "\\$1")
                            });
                            return A
                        }).replace(/(?="|')/g, "\\").replace(q.query(), function (A) {
                            A = A.replace(y, "").replace(z, "");
                            return '";' + A.replace(/\\/g, "") + ';view+="'
                        }).replace(q.query(1), function (A) {
                            var B = '"+(';
                            if (A.replace(/\s/g, "") === i + t) {
                                return ""
                            }
                            A = A.replace(q.exp(i + "|" + t), "");
                            if (/^=/.test(A)) {
                                A = A.replace(/^=/, "");
                                B = '"+_escape_('
                            }
                        ...
                        v = '"use strict";var view = "' + v + '";return view;';
                        v = new Function("d, _escape_", v);
                        return v(w, q.escape)
                    } catch (x) {
                        q.error(x, u);
                        return u
            ...
            if (typeof q.url === "string") {
                q.success = function (r) {
                    n.util.render(h.extend({}, q, {url: r}))
                };
                if (q.ajax === "ajax") {
                    n.ajax(q)
                } else {
                    n.req(q.url, q.where, q.success, q.method, q)
                }
                return
            }
            var i = n.util.tpl(q.tpl, q.url, q.open || d.tplOpen, q.close || d.tplClose);
            h(q.elem).next("[ew-tpl-rs]").remove();
            h(q.elem).after(i);
            h(q.elem).next().attr("ew-tpl-rs", "");
            q.done && q.done(q.url)
        }
    };
    n.lockScreen = function (r) {
        if (!r) {
            r = "components/tpl/lock-screen.html"
        }
        ...
        if (i.length > 0) {
            ...
            var q = k.load(2);
            n.ajax({
                url: r, dataType: "html", success: function (s) {
                    k.close(q);
                    if (typeof s === "string") {
                        h("body").append('<div id="ew-lock-screen-group">' + s + "</div>");
                        n.isLockScreen = true;
                        n.putTempData("isLockScreen", n.isLockScreen, true);
                        n.putTempData("lockScreenUrl", r, true)
                    } else {
                        console.error(s);
                        k.msg(JSON.stringify(s), {icon: 2, anim: 6})
                    }
                }
            })
        }
    };
    n.unlockScreen = function (q) {
        var i = h("#ew-lock-screen-group");
        q ? i.remove() : i.fadeOut("fast");
    ...
    n.tips = function (i) {
        return k.tips(i.text, i.elem, {
            ...
            time: i.time || -1,
            success: function (q) {
                var r = h(q).children(".layui-layer-content");
                if (i.padding || i.padding === 0) {
                    r.css("padding", i.padding)
                }
                if (i.color) {
                    r.css("color", i.color)
                }
                if (i.bgImg) {
                    r.css("background-image", i.bgImg).children(".layui-layer-TipsG").css("z-index", "-1")
                }
                if (i.fontSize) {
                ...
                var u = i.offset.split(",");
                var t = u[0], s = u.length > 1 ? u[1] : undefined;
                if (t) {
                    h(q).css("margin-top", t)
                }
                if (s) {
                    h(q).css("margin-left", s)
                }
            }
        })
    };
    n.renderTpl = function (q) {
        if (!layui.admin) {
            layui.admin = n
        }
        ...
        h(q || "[ew-tpl]").each(function () {
            var t = h(this);
            var r = n.util.deepClone(h(this).data());
            r.elem = t;
            r.tpl = t.html();
            r.url = i(t.attr("ew-tpl"));
            r.headers = i(r.headers);
            r.where = i(r.where);
            if (r.done) {
                try {
                    r.done = new Function("res", r.done)
                } catch (s) {
                    console.error(s + "\nlay-data:" + r.done);
                    r.done = undefined
                }
    ...
    n.on = function (i, q) {
        return layui.onevent.call(this, "admin", i, q)
    };
    n.putSetting = function (i, q) {
        d[i] = q;
        n.putTempData(i, q, true)
    };
    ...
    h(document).on("mouseenter", m + "," + m + " .layui-nav-child>dd", function () {
        if (n.getPageWidth() > 768) {
            var i = h(this), r = i.find(">.layui-nav-child");
            if (r.length > 0) {
    ...
    h(document).on("click.ew-sel-fixed", ".ew-select-fixed .layui-form-select .layui-select-title", function () {
        var w = h(this), s = w.parent().children("dl"), u = w.offset().top;
        var t = w.outerWidth(), y = w.outerHeight(), i = h(document).scrollTop();
        var v = s.outerWidth(), r = s.outerHeight();
        var x = u + y + 5 - i, q = w.offset().left;
        if (x + r > n.getPageHeight()) {
            x = x - r - y - 10
        }
        if (q + v > n.getPageWidth()) {
            q = q - v + t
        }
    ...
    h(document).on("click", ".layui-nav-tree>.layui-nav-item a", function () {
        var s = h(this), q = s.siblings(".layui-nav-child"), r = s.parent();
        if (q.length === 0) {
            return
        }
    ...
    h(document).on("click", ".layui-collapse>.layui-colla-item>.layui-colla-title", function () {
        var t = h(this), i = t.siblings(".layui-colla-content"), r = t.parent().parent(), q = i.hasClass("layui-show");
        if (q) {
    ...
    k.tips = function (s, i, r) {
        ...
        if (!q) {
            return k.oldTips(s, i, r)
        }
        r.tips = [q.attr("lay-direction") || 3, q.attr("lay-bg") || "#ff4c4c"];
        setTimeout(function () {
            r.success = function (t) {
                h(t).children(".layui-layer-content").css("padding", "6px 12px")
            };
            k.oldTips(s, i, r)
        }, 100)
    };
    ...
    if (a) {
        var p = ["pageTabs", "cacheTab", "defaultTheme", "navArrow", "closeFooter", "tabAutoRefresh"];
